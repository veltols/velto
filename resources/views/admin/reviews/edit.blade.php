...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white shadow-xs rounded-xl border border-gray-200 overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between">
            <div>
                <h2 class="text-lg font-bold text-gray-900">Edit Customer Review #{{ $review->id }}</h2>
                <p class="mt-1 text-xs text-gray-500">Submitted by {{ $review->customer_name }} on {{ $review->created_at ? $review->created_at->format('M d, Y') : 'Unknown' }}</p>
            </div>
            <span class="inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold {{ $review->getStatusBadgeClass() }}">
                {{ ucfirst($review->status) }}
            </span>
        </div>

        <form action="{{ route('admin.reviews.update', $review) }}" method="POST" class="p-6 space-y-6" x-data="{ currentRating: {{ old('rating', $review->rating) }} }">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="text" name="customer_name" id="customer_name" value="{{ old('customer_name', $review->customer_name) }}" required class="mt-1.5 block w-full rounded-md border-gray-300 text-sm shadow-xs focus:border-black focus:ring-black">
                    @error('customer_name')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="email" name="customer_email" id="customer_email" value="{{ old('customer_email', $review->customer_email) }}" class="mt-1.5 block w-full rounded-md border-gray-300 text-sm shadow-xs focus:border-black focus:ring-black">
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $review->title) }}" required class="mt-1.5 block w-full rounded-md border-gray-300 text-sm shadow-xs focus:border-black focus:ring-black">
                @error('title')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" required class="mt-1.5 block w-full rounded-md border-gray-300 text-sm shadow-xs focus:border-black focus:ring-black">{{ old('description', $review->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-3 pt-2 border-t border-gray-100">
                ...
                <div>
                    ...
                    <select name="status" id="status" class="mt-1.5 block w-full rounded-md border-gray-300 text-sm shadow-xs focus:border-black focus:ring-black">
                        <option value="approved" {{ old('status', $review->status) === 'approved' ? 'selected' : '' }}>Approved (Visible on site)</option>
                        <option value="pending" {{ old('status', $review->status) === 'pending' ? 'selected' : '' }}>Pending (Needs review)</option>
                        <option value="rejected" {{ old('status', $review->status) === 'rejected' ? 'selected' : '' }}>Rejected (Hidden)</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex items-center pt-6">
                    <label class="relative flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="verified_purchase" value="1" {{ old('verified_purchase', $review->verified_purchase) ? 'checked' : '' }} class="h-4 w-4 rounded border-gray-300 text-black focus:ring-black">
                ...
                <div class="flex items-center pt-6">
                    <label class="relative flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $review->is_featured) ? 'checked' : '' }} class="h-4 w-4 rounded border-gray-300 text-black focus:ring-black">
            ...
            <div class="flex items-center justify-between pt-6 border-t border-gray-100">
                <button type="button" onclick="if(confirm('Delete this review permanently?')) document.getElementById('delete-review-form').submit();" class="text-sm font-medium text-red-600 hover:text-red-800">
                    Delete Review
                </button>
                <div class="flex items-center gap-3">
                    ...
                    <button type="submit" class="rounded-md bg-black px-5 py-2 text-sm font-semibold text-white shadow-sm hover:bg-gray-800 transition">Update Review</button>
                </div>
            </div>
        </form>
