...
<div class="max-w-2xl mx-auto">
    <form action="{{ route('admin.banners.update', $banner) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <div class="mt-2">
                <textarea name="text" id="text" rows="3" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-black sm:text-sm sm:leading-6">{{ old('text', $banner->text) }}</textarea>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="mt-2">
                @if($banner->image_path)
                    <div class="mb-2">
                        <img src="{{ Storage::url($banner->image_path) }}" alt="" class="h-20 w-auto rounded">
                    </div>
                @endif
